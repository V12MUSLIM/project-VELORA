...
const mockGeminiAPI = async (query, product, reviews) => {
  ...
  if (q.includes("summary") || q.includes("summarize reviews")) {
    if (reviews.length === 0)
      return "There are no reviews yet for this product.";
    const totalRating =
      reviews.reduce((acc, r) => acc + r.rating, 0) / reviews.length;
    const goodPoints = reviews
      .filter((r) => r.rating >= 4)
      .map((r) => r.title || r.comment.substring(0, 50) + "...")
      .slice(0, 2);
  ...
  if (q.includes("specifications") || q.includes("specs")) {
    if (!product.specs || product.specs.length === 0)
      return "There are no specifications listed for this product.";
        ...
        >
          {messages.map((msg, index) => (
            ...
            >
              ...
              <div
                className={`max-w-xs md:max-w-md p-3 rounded-2xl ${msg.sender === "user" ? "bg-primary text-white rounded-br-none" : "bg-gray-100 dark:bg-gray-800 rounded-bl-none"}`}
              >
                {msg.text}
              </div>
            </div>
